...
class PlgUserUsernameCheck extends JPlugin
{
	...
	public function onUserBeforeSave($user, $isNew, $data)
	{
		...
		{
			$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_MINNUMCHARS_REQUIRED', $minNumChars, $usernameLenght), 'warning');
			$result = false;
		}
		...
		{
			$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_MAXNUMCHARS_REQUIRED', $maxNumChars, $usernameLenght), 'warning');
			$result = false;
		}
		...
		if ($allowed_preset)
		{
			switch ($allowed_preset)
			{
				...
				case 2: // CUSTOM FORBIDDEN
					...
					{
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_CHARSET_REQUIRED', implode(' ', $invalid_chars)), 'warning');
						$result = false;
					}
					break;
				...
				case 3:
					...
					if (preg_match_all($regExp, $value, $nonRegExpChars))
					{
						...
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_CHARSET_REQUIRED', $nonRegExpString), 'warning');
				...
				case 4:
					...
					{
						// Enqueue error message and return false
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_ALPHANUMERIC_REQUIRED'), 'warning');
				...
				case 5:
					...
					if (preg_match_all('/[^\\p{Common}\\p{Latin}]/u', $value, $nonLatinChars))
					{
						...
						// Enqueue error message and return false
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_LATIN_REQUIRED', $nonLatinString), 'warning');
				...
				case 6:
					...
					// Si el username proviene de más de un script recomendado (o combinación permitida) lanza error
					if (!preg_match_all($regexRecommendedScripts, $valueWithoutCommonInherited))
					{
						// Enqueue error message and return false
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_ONESCRIPT_REQUIRED'), 'warning');
...
\x{2019}\x{2027}\x{30A0}\x{30FB}]/u";
					...
					if (preg_match_all($regexAllowedChars, $value, $nonIdentifierProfile))
					{
						...
						// Enqueue error message and return false
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_IDPROFILE_REQUIRED', $nonIdentifierProfileString), 'warning');
					...
					* {
					*	// Enqueue error message and return false
					*	$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_IDSTART_REQUIRED'), 'warning');
					*
					*	$result = false;
					* }
					*
					...
					*
					*	// Enqueue error message and return false
					*	$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_IDCONTINUE_REQUIRED', $nonIdContinueString), 'warning');
					*
					*	$result = false;
					* }
					*/
				...
				case 7:
					...
					{
						// Enqueue error message and return false
						$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_EMAIL_REQUIRED', implode(' ', $invalid_chars)), 'warning');
				...
				default:
					// NO OPTION
					$this->app->enqueueMessage(JText::sprintf('PLG_USER_USERNAMECHECK_FIELD_USERNAME_NOOPTION'), 'warning');
